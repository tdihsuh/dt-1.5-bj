...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      let url =
        `/service/api/credit/operation/enterprise/list?` +
        queryString +
        `pageNum=${this.current}&limitSize=${this
          .pageSize}&_t=${new Date().valueOf()}`;
      if (this.$route.query.type === "person") {
        this.isPersonal = true;
        this.columns = personalColumns;
        url =
          `/service/api/credit/operation/person/list?` +
          queryString +
          `pageNum=${this.current}&limitSize=${this
            .pageSize}&_t=${new Date().valueOf()}`;
      } else {
